...
PARAM$dataset_nom <-("./competencia_01_python.csv")
...
setwd("C:/Users/maria/") # Establezco el Working Directory
# cargo los datos
...
# itero por los loops anidados para cada hiperparametroyyy
for (vcp in c(-0.5)) {#saco-0.5,-0.1,0.1 y dejo el 0
  for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
    for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
      for (vmin_bucket in c(5, 10, 15, 20)) {
        
        # Aquí, asegúrate de que cada hiperparámetro toma el valor de la variable del bucle
        param_basicos <- list(
        ...
        # Llamada a Montecarlo con los parámetros actuales
        ganancias <- ArbolesMontecarlo(PARAM$semillas, param_basicos)
        
        # Agrego a la tabla los resultados
        tb_grid_search_detalle <- rbindlist(
          list(tb_grid_search_detalle, rbindlist(ganancias))
        )
        
        # Opcional: Imprimir progreso para seguimiento
        cat("Iteración completada con: cp =", vcp, 
            "maxdepth =", vmax_depth, 
            "minsplit =", vmin_split, 
